...
public class CSCAN extends android.app.Activity{
    ...
    public String[] runAlgorithm() {
        // it's like an elevator, but only in one direction
        ...
        while (!exit) {
            ...
            for(int i = 0; i < reqList.size(); i++){
                if((reqList.get(i).getTrackRequested() == currentTrack) && (reqList.get(i).getArrivalTime() <= currentTime) && (reqList.get(i).getComplete() == false)){
            ...
            //cycle at boundaries
            if (currentTrack == 255) { //go back to track 0
                currentTrack = 0;
            }
